import {
  ...
  export default function From1EditActiveAdmin(props) {
    ...
    const {
      ...
      createdAt,
      updatedAt,
    } = props.route.params.from1EditActiveAdmin;
    ...
    const [nameActive, setNameActive] = useState(act_name);
    const [description, setDescription] = useState(act_description);
    const [price, setPrice] = useState(act_price);
  
    const [openOrganize, setOpenOrganize] = useState(false); // open and close the modal
    const [dateOrganize, setdateOrganize] = useState(formatActTime``);
  
    const formatActTime = formatTime(dateOrganize)
    ...
    const navigateFormContinue = () => {
      if (nameActive === "" || dateOrganize === "DD/MM/YYYY") {
        ...
        navigation.navigate("from2EditActiveAdmin", {
          locationFromForm1: locationFromDetail,
          organizerFromForm1: organizerFromDetail,
          costFromForm1: costFromDetail,
          descriptionFromForm1: descriptionFromDetail,
        });
      }
    };
