import {
  ...
  export default function From1EditActiveDoanTruong(props) {
    const {navigation} = props
    const {
      nameActiveFromDetail,
      timeOrganizeFromDetail,
      deadlineFromDetail,
      locationFromDetail,
      organizerFromDetail,
      costFromDetail,
    ...
    const [nameActive, setNameActive] = useState(nameActiveFromDetail);
  
    const [openOrganize, setOpenOrganize] = useState(false); // open and close the modal
    const [dateOrganize, setdateOrganize] = useState(timeOrganizeFromDetail);
    ...
    const [dateDeadLine, setdateDeadLine] = useState(deadlineFromDetail);
    ...
    const navigateFormContinue = () => {
      ...
      // sẽ đc hệ thống nhắc nhở
        ...
        navigation.navigate('from2EditActiveDoanTruong', { 
