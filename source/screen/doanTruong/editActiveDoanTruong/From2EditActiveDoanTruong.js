...
import EditActiveAction from "../../../redux/action/removeEditActiveAction/EditActiveAction";
...
export default function From2EditActiveDoanTruong(props) {
  const dispatch = useDispatch()
  const {navigation} = props
  const { loading, reponseSuccess, error } = useSelector(
    (state) => state.editActiveReducer
  );
  ...
  const [location, setLocation] = useState(act_address);

  const [organizer, setOrganizer] = useState(organizationOld);
  ...
  const [cost, setCost] = useState(`${act_price}`);

  const [description, setDescription] = useState(act_description);
  ...
  const navigateCreateActive = () => {
    if (location === "" || organizer === "") {
      ...
      const editActive = {
        ...
        // vì khi sửa lại hoạt động thì phải chờ duyệt lại
        act_status: 1,
      };

      dispatch(EditActiveAction(editActive));
    }
  };

  useEffect(() => {
    dispatch({ type: EDIT_ACTIVE_RESET });
  }, [dispatch]);
  ...
  useEffect(() => {
    if (error != null && loading == false) {
      ...
      Alert.alert("Bạn đã sửa hoạt động thành công");
      navigation.dispatch(
        CommonActions.reset({
          index: 0,
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.containerHeader}>
          <Text style={styles.header}>Sửa hoạt động</Text>
        </View>
        ...
        >
          ...
          >
            <Text style={styles.resigter}>Sửa hoạt động</Text>
          </TouchableOpacity>
        </View>
      </ImageBackground>
    </View>
